...
std::optional<bool> call_object_bool_function(sdk::UObject* object, std::wstring_view function_name);
std::optional<uint8_t> call_object_uint8_function(sdk::UObject* object, std::wstring_view function_name);
std::optional<float> call_object_float_function(sdk::UObject* object, std::wstring_view function_name);
}
...
bool is_windrose_executable_cached() {
    static const bool is_windrose = []() {
        ...
        return lowered.find(L"windrose-win64-shipping") != std::wstring::npos;
    }();
...
bool is_runtime_object_full_name(std::wstring_view full_name) {
    ...
    return full_name.rfind(L"Class ", 0) != 0 &&
           full_name.rfind(L"WidgetBlueprintGeneratedClass ", 0) != 0 &&
           full_name.rfind(L"BlueprintGeneratedClass ", 0) != 0 &&
           full_name.rfind(L"AnimBlueprintGeneratedClass ", 0) != 0 &&
           full_name.rfind(L"Package ", 0) != 0 &&
           full_name.rfind(L"Function ", 0) != 0 &&
           full_name.rfind(L"DelegateFunction ", 0) != 0 &&
           full_name.rfind(L"Enum ", 0) != 0 &&
           full_name.rfind(L"ScriptStruct ", 0) != 0 &&
           full_name.rfind(L"WidgetTree ", 0) != 0 &&
           full_name.rfind(L"WidgetAnimation ", 0) != 0 &&
           full_name.rfind(L"MovieScene ", 0) != 0 &&
           full_name.rfind(L"MovieSceneWidgetMaterialTrack ", 0) != 0 &&
           full_name.rfind(L"MovieSceneParameterSection ", 0) != 0 &&
           full_name.rfind(L"Image ", 0) != 0 &&
           full_name.rfind(L"TextBlock ", 0) != 0 &&
           full_name.rfind(L"Button ", 0) != 0 &&
           full_name.rfind(L"Border ", 0) != 0 &&
           full_name.rfind(L"Overlay ", 0) != 0 &&
           full_name.rfind(L"CanvasPanel ", 0) != 0 &&
           full_name.rfind(L"SizeBox ", 0) != 0 &&
           full_name.rfind(L"VerticalBox ", 0) != 0 &&
           full_name.rfind(L"HorizontalBox ", 0) != 0 &&
           !contains_case_insensitive(full_name, L"Default__");
}

std::string windrose_r5_ui_log_sample(std::wstring_view full_name) {
    constexpr size_t max_chars = 180;
    auto sample = full_name.size() > max_chars
        ? std::wstring{full_name.substr(0, max_chars)} + L"..."
        : std::wstring{full_name};

    return sample.empty() ? "unresolved" : utility::narrow(sample);
}
...
bool windrose_r5_ui_class_candidate(std::wstring_view class_name) {
    return contains_case_insensitive(class_name, L"BP_SimpleUIMounter_SC") ||
           ...
           contains_case_insensitive(class_name, L"BP_CraftUIMounter_") ||
           contains_case_insensitive(class_name, L"BP_NPC_ViewAll_SC") ||
           contains_case_insensitive(class_name, L"BP_NPC_Dispatcher_SC") ||
           contains_case_insensitive(class_name, L"BP_NPC") ||
           contains_case_insensitive(class_name, L"BP_ShipDock_Shipyard_SC") ||
           contains_case_insensitive(class_name, L"BP_HFSM_FastTravel_Component") ||
           ...
           contains_case_insensitive(class_name, L"BP_HFSM_ShipDock") ||
           contains_case_insensitive(class_name, L"BP_HFSM_ShipManager") ||
           contains_case_insensitive(class_name, L"BP_HFSM_ShipManage_") ||
           contains_case_insensitive(class_name, L"BP_HFSM_ShipInventory") ||
           contains_case_insensitive(class_name, L"BP_HFSM_BuildingCenter") ||
...
bool windrose_r5_ui_name_is_gameplay_meta_hfsm(std::wstring_view value) {
    ...
    const auto is_gameplay_meta_path =
        contains_case_insensitive(value, L"/Game/UI/System/HFSM/OverlayUI/META/") ||
        contains_case_insensitive(value, L"/Game/UI/System/HFSM/OverlayUI/ShipMeta/");

    const auto is_gameplay_dialogue =
        contains_case_insensitive(value, L"BP_HFSM_ScenarioDialogue") ||
        contains_case_insensitive(value, L"R5UIDialogueController");

    if (!is_gameplay_meta_path && !is_gameplay_dialogue) {
        return false;
    }

    return contains_case_insensitive(value, L"BP_HFSM_MetaCharacter") ||
           ...
           contains_case_insensitive(value, L"BP_HFSM_LootStorage") ||
           contains_case_insensitive(value, L"BP_HFSM_WaterLootStorage") ||
           contains_case_insensitive(value, L"BP_HFSM_Storage") ||
           contains_case_insensitive(value, L"BP_HFSM_PosthumousContainer") ||
           contains_case_insensitive(value, L"BP_HFSM_Rarities") ||
           contains_case_insensitive(value, L"BP_HFSM_ShipInteraction") ||
...
bool windrose_r5_ui_name_is_gameplay_fullscreen_meta(std::wstring_view value) {
    ...
    if (contains_case_insensitive(value, L"Lobby") ||
        contains_case_insensitive(value, L"MainMenu") ||
        contains_case_insensitive(value, L"World_Select") ||
        contains_case_insensitive(value, L"WorldSelect") ||
        contains_case_insensitive(value, L"Difficulty") ||
    ...
    // These are the R5 UI routes that reference BP_NegativeSpaceController_SC.
    // Use them as the trigger, and the controller/widget as the target. This
    // avoids broad scene suppression while still covering the non-map tabs that
    // flicker when the game's own negative-space layer is not stable in HMD.
    return contains_case_insensitive(value, L"BP_HFSM_MetaUI") ||
           ...
           contains_case_insensitive(value, L"BP_HFSM_Craft_") ||
           contains_case_insensitive(value, L"BP_HFSM_Trade_") ||
           contains_case_insensitive(value, L"BP_HFSM_BuildingCenter") ||
           contains_case_insensitive(value, L"BP_HFSM_ScenarioDialogue") ||
           contains_case_insensitive(value, L"BP_HFSM_ScenarioHiring") ||
           contains_case_insensitive(value, L"BP_HFSM_ShipDock") ||
           contains_case_insensitive(value, L"BP_HFSM_ShipManager") ||
           contains_case_insensitive(value, L"BP_HFSM_ShipManage_") ||
           contains_case_insensitive(value, L"BP_HFSM_ShipInventory") ||
           contains_case_insensitive(value, L"BP_HFSM_FastTravel_Component") ||
           contains_case_insensitive(value, L"BP_CraftUIMounter_") ||
           contains_case_insensitive(value, L"BP_NPC_ViewAll_SC") ||
           contains_case_insensitive(value, L"BP_NPC_Dispatcher_SC") ||
           contains_case_insensitive(value, L"BP_NPC") ||
           contains_case_insensitive(value, L"BP_ShipDock_Shipyard_SC") ||
           ...
           contains_case_insensitive(value, L"WBP_NPC") ||
           contains_case_insensitive(value, L"WBP_Craft") ||
           contains_case_insensitive(value, L"WBP_Cooking") ||
           contains_case_insensitive(value, L"WBP_Cook") ||
           contains_case_insensitive(value, L"WBP_Dialog") ||
           contains_case_insensitive(value, L"WBP_Chat") ||
           contains_case_insensitive(value, L"WBP_ShipManage_");
}

bool windrose_r5_ui_name_excluded_from_modal(std::wstring_view value) {
    return contains_case_insensitive(value, L"Build") ||
           contains_case_insensitive(value, L"Building") ||
           contains_case_insensitive(value, L"Construction") ||
           contains_case_insensitive(value, L"Construct") ||
           contains_case_insensitive(value, L"Placement") ||
           contains_case_insensitive(value, L"Snap") ||
           contains_case_insensitive(value, L"/Game/UI/HUD/") ||
           contains_case_insensitive(value, L"/Game/UI/System/HFSM/") ||
           contains_case_insensitive(value, L"BP_R5HUD") ||
           contains_case_insensitive(value, L"BP_HUDComponent") ||
           contains_case_insensitive(value, L"GlobalStorage") ||
           contains_case_insensitive(value, L"_GEN_VARIABLE") ||
           contains_case_insensitive(value, L"WBP_RangedWeapon_ShotIndicator") ||
           contains_case_insensitive(value, L"ShotIndicator") ||
           contains_case_insensitive(value, L"RangedWeapon") ||
           contains_case_insensitive(value, L"MapMarker") ||
           contains_case_insensitive(value, L"MiniMap") ||
           contains_case_insensitive(value, L"/Game/UI/CommonElements/") ||
           contains_case_insensitive(value, L"/Game/UI/Settings/") ||
           contains_case_insensitive(value, L"WBP_ContainerButton") ||
           contains_case_insensitive(value, L"GamepadHint") ||
           contains_case_insensitive(value, L"HintContainer") ||
           contains_case_insensitive(value, L"WBP_BuildVersion") ||
           contains_case_insensitive(value, L"Lobby") ||
           contains_case_insensitive(value, L"ServerPassword") ||
           contains_case_insensitive(value, L"MainMenu");
}

bool windrose_r5_ui_name_is_asset_or_child_widget(std::wstring_view value) {
    if (contains_case_insensitive(value, L".WidgetTree.") ||
        contains_case_insensitive(value, L" WidgetTree.") ||
        contains_case_insensitive(value, L"WidgetTree_") ||
        contains_case_insensitive(value, L".uw_") ||
        contains_case_insensitive(value, L".txt_") ||
        contains_case_insensitive(value, L".img_") ||
        contains_case_insensitive(value, L".btn_") ||
        contains_case_insensitive(value, L".slot_") ||
        contains_case_insensitive(value, L".vbox_") ||
        contains_case_insensitive(value, L".hbox_")) {
        return true;
    }

    if (contains_case_insensitive(value, L"/Widgets/Slots/") ||
        contains_case_insensitive(value, L"WBP_InventorySlot") ||
        contains_case_insensitive(value, L"WBP_Inventory_Item") ||
        contains_case_insensitive(value, L"InventorySlot") ||
        contains_case_insensitive(value, L"WBP_Craft_BottomBar") ||
        contains_case_insensitive(value, L"/Crafting/Generic/")) {
        return true;
    }

    if (contains_case_insensitive(value, L"/Game/UI/META/Rarities/") ||
        contains_case_insensitive(value, L"RarityParams") ||
        contains_case_insensitive(value, L"QuestRarityParams") ||
        contains_case_insensitive(value, L"ItemFilterParams") ||
        contains_case_insensitive(value, L"DataAsset") ||
        contains_case_insensitive(value, L"DataTable")) {
        return true;
    }

    // `/Game/UI/...` objects are usually blueprint assets/templates. Live UMG
    // instances are normally transient or world-owned, and are much safer modal signals.
    if (contains_case_insensitive(value, L" /Game/UI/") &&
        !contains_case_insensitive(value, L"/Engine/Transient") &&
        !contains_case_insensitive(value, L"PersistentLevel") &&
        !contains_case_insensitive(value, L"GameInstance") &&
        !contains_case_insensitive(value, L"GameViewport")) {
...
bool windrose_r5_ui_name_is_top_level_modal_shell(std::wstring_view value) {
    if (contains_case_insensitive(value, L"R5SC_MetaMenuTabs") ||
        contains_case_insensitive(value, L"R5MetaMenuTabsVM") ||
        contains_case_insensitive(value, L"R5UIDialogueController")) {
        return true;
    }

    const auto is_wbp_root =
        contains_case_insensitive(value, L"WBP_") &&
        !contains_case_insensitive(value, L".WidgetTree.") &&
        !contains_case_insensitive(value, L"/Widgets/");

    if (!is_wbp_root) {
        return false;
    }

    return contains_case_insensitive(value, L"WBP_MetaMenu") ||
           contains_case_insensitive(value, L"WBP_MetaCharacter_Tabs") ||
           contains_case_insensitive(value, L"WBP_FullscreenMap") ||
           contains_case_insensitive(value, L"WBP_Inventory") ||
           contains_case_insensitive(value, L"WBP_InventoryDefaultAndAction_Panel") ||
           contains_case_insensitive(value, L"WBP_Equipment") ||
           contains_case_insensitive(value, L"WBP_PlayerEquipment_Panel") ||
           contains_case_insensitive(value, L"WBP_Character_Progression") ||
           contains_case_insensitive(value, L"WBP_Adventure") ||
           contains_case_insensitive(value, L"WBP_Discovery_Screen") ||
           contains_case_insensitive(value, L"WBP_TalentTree") ||
           contains_case_insensitive(value, L"WBP_ShipManage_") ||
           contains_case_insensitive(value, L"WBP_CraftTrade_") ||
           contains_case_insensitive(value, L"WBP_CraftProduction_") ||
           contains_case_insensitive(value, L"WBP_CraftGear_") ||
           contains_case_insensitive(value, L"WBP_CraftEquipment_") ||
           contains_case_insensitive(value, L"WBP_Journal") ||
           contains_case_insensitive(value, L"WBP_Cooking") ||
           contains_case_insensitive(value, L"WBP_Cook") ||
           contains_case_insensitive(value, L"WBP_Recipe") ||
           contains_case_insensitive(value, L"WBP_Dialog") ||
           contains_case_insensitive(value, L"WBP_Dialogue") ||
           contains_case_insensitive(value, L"WBP_Chat");
}

bool windrose_r5_ui_name_is_meta_menu_controller(std::wstring_view value) {
    if (windrose_r5_ui_name_is_gameplay_meta_hfsm(value)) {
        return true;
    }

    if (windrose_r5_ui_name_excluded_from_modal(value) ||
        windrose_r5_ui_name_is_asset_or_child_widget(value)) {
        return false;
    }

    // The K-opened character/inventory menu can be active before the pooled
    // root widget exposes a viewport/visibility state. Treat the gameplay
    // meta-menu controllers/VMs as the modal signal, but keep it limited to
    // transient/world-owned runtime objects.
    return contains_case_insensitive(value, L"R5SC_MetaMenuTabs") ||
           contains_case_insensitive(value, L"R5MetaMenuTabsVM") ||
           contains_case_insensitive(value, L"R5DefaultInventoriesScreenHFSMComponent") ||
           contains_case_insensitive(value, L"R5HFSMInventoryItemsInfoComponent") ||
           contains_case_insensitive(value, L"R5BaseInventoryVM") ||
...
bool windrose_r5_hfsm_state_has_active_time(sdk::UObject* object) {
    const auto active_time = call_object_float_function(object, L"GetActiveTime");
    // GCFSM exposes active time on the currently entered state. Keep a wide
    // upper bound so stale/corrupt values do not pin the detector on forever.
    return active_time.has_value() && *active_time > 0.0f && *active_time < 24.0f * 60.0f * 60.0f;
}

bool windrose_r5_ui_hfsm_object_is_active(sdk::UObject* object) {
    if (!is_valid_uobject_basic(object)) {
        return false;
    }

    if (windrose_r5_hfsm_state_has_active_time(object)) {
        return true;
    }

    const auto state = read_object_property(object, L"State").value_or(nullptr);
    return is_valid_uobject_basic(state) && state != object && windrose_r5_hfsm_state_has_active_time(state);
}

bool windrose_r5_ui_instance_is_active(sdk::UObject* object) {
    const auto in_viewport = call_object_bool_function(object, L"IsInViewport");
    const auto visible = call_object_bool_function(object, L"IsVisible");
    const auto slate_visibility = call_object_uint8_function(object, L"GetVisibility");

    if (visible.value_or(false)) {
        return true;
    }

    if (slate_visibility.has_value()) {
        // ESlateVisibility: 0=Visible, 3=HitTestInvisible, 4=SelfHitTestInvisible.
        // R5 keeps many menu widgets pooled, so IsInViewport can be false while
        // the pooled widget is still the visible menu surface.
        if (*slate_visibility == 0 || *slate_visibility == 3 || *slate_visibility == 4) {
            return true;
        }
    }

    if (in_viewport.has_value()) {
        return *in_viewport;
    }

    return windrose_r5_ui_hfsm_object_is_active(object);
}

bool windrose_r5_ingame_2d_menu_ui_name(std::wstring_view value) {
    if (windrose_r5_ui_name_is_gameplay_meta_hfsm(value)) {
        return true;
    }
    ...
    return windrose_r5_ui_name_is_top_level_modal_shell(value);
}

bool windrose_r5_modal_ui_name(std::wstring_view value) {
    return windrose_r5_ingame_2d_menu_ui_name(value);
}

bool windrose_r5_scene_blackout_ui_name(std::wstring_view value) {
    return windrose_r5_ingame_2d_menu_ui_name(value);
}
...
std::optional<uint8_t> call_object_uint8_function(sdk::UObject* object, std::wstring_view function_name) try {
    if (object == nullptr || IsBadReadPtr(object, sizeof(void*))) {
        return std::nullopt;
    }
...
std::optional<float> call_object_float_function(sdk::UObject* object, std::wstring_view function_name) try {
    if (object == nullptr || IsBadReadPtr(object, sizeof(void*))) {
        return std::nullopt;
    }
...
void VR::record_hitch_snapshot_sample(std::chrono::steady_clock::time_point now) {
    ...
    if (const auto runtime = get_runtime(); runtime != nullptr && runtime->is_openxr()) {
        if (const auto openxr = get_openxr_runtime(); openxr != nullptr) {
            ...
            sample.stale_pose_older_frame_skip_count = openxr->stale_pose_older_frame_skip_count;
            sample.stale_pose_older_frame_skip_suppressed_count = openxr->stale_pose_skip_suppressed_total;
            sample.stale_pose_refresh_attempt_count = openxr->stale_pose_refresh_attempt_count;
            sample.stale_pose_refresh_success_count = openxr->stale_pose_refresh_success_count;
            sample.stale_pose_refresh_failed_count = openxr->stale_pose_refresh_failed_count;
            sample.focused_frame_loop_recovery_count = openxr->focused_frame_loop_recovery_count;
            sample.forced_frame_recovery_count = openxr->forced_frame_recovery_count;
            sample.last_pose_update_frame_count = openxr->last_pose_update_frame_count;
        }
    }

    if (m_fake_stereo_hook != nullptr) {
        if (const auto rtm = m_fake_stereo_hook->get_render_target_manager(); rtm != nullptr) {
            sample.scene_capture_destroyed_count = rtm->get_scene_capture_destroyed_count();
        }
    }
...
void VR::write_hitch_snapshot_request(HitchSnapshotDumpRequest&& request) try {
    ...
    HitchSnapshotSample last_valid_sample{};
    bool has_last_valid_sample = false;
    ...
    for (const auto& sample : request.samples) {
        ...
        samples.push_back({
            ...
            {"stale_pose_older_frame_skip_count", sample.stale_pose_older_frame_skip_count},
            {"stale_pose_older_frame_skip_suppressed_count", sample.stale_pose_older_frame_skip_suppressed_count},
            {"stale_pose_refresh_attempt_count", sample.stale_pose_refresh_attempt_count},
            {"stale_pose_refresh_success_count", sample.stale_pose_refresh_success_count},
            {"stale_pose_refresh_failed_count", sample.stale_pose_refresh_failed_count},
            {"focused_frame_loop_recovery_count", sample.focused_frame_loop_recovery_count},
            {"forced_frame_recovery_count", sample.forced_frame_recovery_count},
            {"last_pose_update_frame_count", sample.last_pose_update_frame_count},
            {"scene_capture_destroyed_count", sample.scene_capture_destroyed_count},
    ...
    if (has_last_valid_sample) {
        const auto& sample = last_valid_sample;
        const auto& d3d12 = sample.d3d12;
        json summary{
            {"type", "uevr_hitch_snapshot_summary"},
            {"source", request.path.filename().string()},
            {"tick_gap_ms", request.tick_gap_ms},
            {"suspected_stall", request.suspected_stall},
            {"sample_count", root["sample_count"].get<size_t>()},
            {"last_sample", {
                {"frame_count", sample.frame_count},
                {"render_frame_count", sample.render_frame_count},
                {"runtime_ready", sample.runtime_ready},
        ...
        std::ofstream summary_file{request.path.parent_path() / "hitch_snapshot_latest_summary.json"};
        summary_file << summary.dump(2);
    }

    prune_hitch_snapshots(request.path.parent_path());
...
void VR::prune_hitch_snapshots(const std::filesystem::path& dir) {
    try {
        ...
        for (const auto& entry : std::filesystem::directory_iterator{dir}) {
            ...
            if (filename.starts_with("hitch_snapshot_") && filename.ends_with(".json") &&
                filename != "hitch_snapshot_latest_summary.json")
            {
                snapshots.push_back(entry);
            }
        }

        if (snapshots.size() <= HITCH_SNAPSHOT_MAX_FILES) {
            return;
        }
        ...
        const auto remove_count = snapshots.size() - HITCH_SNAPSHOT_MAX_FILES;
        for (size_t i = 0; i < remove_count; ++i) {
            std::error_code ec{};
            std::filesystem::remove(snapshots[i].path(), ec);
        }
    } catch (const std::exception& e) {
        SPDLOG_WARN("[VR][hitch-snapshot] Failed to prune old snapshots: {}", e.what());
    } catch (...) {
        SPDLOG_WARN("[VR][hitch-snapshot] Failed to prune old snapshots");
    }
}
...
void VR::dump_hitch_snapshot(std::chrono::steady_clock::duration tick_gap, const char* suspected_stall, bool bypass_cooldown) try {
    ...
    if (has_last_valid_sample) {
        m_last_hitch_snapshot_summary = HitchSnapshotSummary{
            .valid = true,
            .path = path,
            .timestamp = now,
            .tick_gap_ms = request.tick_gap_ms,
            .suspected_stall = request.suspected_stall,
            .sample_count = request.samples.size(),
            .last_sample = last_valid_sample,
        };
    }
...
void VR::on_pre_engine_tick(sdk::UGameEngine* engine, float delta) {
            ...
            if (const auto runtime = get_runtime(); runtime != nullptr && runtime->is_openxr()) {
                if (const auto openxr = get_openxr_runtime(); openxr != nullptr) {
                    ...
                    if (openxr->session_state == XR_SESSION_STATE_FOCUSED) {
                        ...
                        const auto openxr_frame_loop_candidate =
                            openxr->session_ready &&
                            openxr->frame_synced &&
                            !openxr->frame_began &&
                            openxr->got_first_poses &&
                            openxr->got_first_valid_poses &&
                            m_d3d12.has_game_and_ui_textures() &&
                            xr_wait_gap_ms > 1000 &&
                            xr_begin_gap_ms > 1000 &&
                            xr_end_gap_ms > 1000;

                        if (openxr_frame_loop_candidate &&
                            (m_last_openxr_recovery_candidate_log.time_since_epoch().count() == 0 ||
                                now - m_last_openxr_recovery_candidate_log >= std::chrono::seconds(10)))
                        {
                            m_last_openxr_recovery_candidate_log = now;
                            spdlog::warn(
                                "[VR][recovery-candidate] Focused OpenXR frame loop is stale with frame_synced=true and frame_began=false; D3D12 textures are present. wait={}ms begin={}ms end={}ms pose={}ms stale_skips={} stale_refresh={}/{}/{} recoveries={}/{}",
                                xr_wait_gap_ms,
                                xr_begin_gap_ms,
                                xr_end_gap_ms,
                                pose_update_gap_ms,
                                openxr->stale_pose_older_frame_skip_count,
                                openxr->stale_pose_refresh_attempt_count,
                                openxr->stale_pose_refresh_success_count,
                                openxr->stale_pose_refresh_failed_count,
                                openxr->focused_frame_loop_recovery_count,
    ...
    update_windrose_r5_ui_state(engine);
...
bool VR::is_windrose_r5_native_modal_ui_phase_lock_active() const {
    return is_windrose_executable_cached() &&
        m_rendering_method->value() == RenderingMethod::NATIVE_STEREO &&
        !is_using_afr() &&
        m_windrose_r5_modal_ui_active.load(std::memory_order_relaxed);
}
...
void VR::note_windrose_r5_ui_copy(
    ...
    const char* reason)
{
    if (!is_windrose_executable_cached() ||
        m_rendering_method->value() != RenderingMethod::NATIVE_STEREO ||
        is_using_afr()) {
        return;
    }

    const auto modal_active = m_windrose_r5_modal_ui_active.load(std::memory_order_relaxed);
    const auto blackout_active = m_windrose_r5_modal_scene_blackout_active.load(std::memory_order_relaxed);
    const auto ui_target_ptr = reinterpret_cast<uintptr_t>(ui_target);
    const auto native_ptr = reinterpret_cast<uintptr_t>(native_resource);
    const auto now = std::chrono::steady_clock::now();

    std::scoped_lock _{m_windrose_r5_ui_copy_mutex};

    const auto should_log =
        !m_windrose_r5_has_ui_copy_sample ||
        ui_target_ptr != m_windrose_r5_last_ui_target ||
        native_ptr != m_windrose_r5_last_ui_native ||
        modal_active != m_windrose_r5_last_ui_copy_modal ||
        blackout_active != m_windrose_r5_last_ui_copy_blackout ||
        copied_to_both_eyes != m_windrose_r5_last_ui_copy_both ||
        is_right_eye_frame != m_windrose_r5_last_ui_copy_right;

    m_windrose_r5_has_ui_copy_sample = true;
    m_windrose_r5_last_ui_copy_frame = frame_count;
    m_windrose_r5_last_ui_target = ui_target_ptr;
    m_windrose_r5_last_ui_native = native_ptr;
    m_windrose_r5_last_ui_copy_modal = modal_active;
    m_windrose_r5_last_ui_copy_blackout = blackout_active;
    m_windrose_r5_last_ui_copy_both = copied_to_both_eyes;
    m_windrose_r5_last_ui_copy_right = is_right_eye_frame;

    if (!should_log) {
        return;
    }

    m_windrose_r5_last_ui_copy_log = now;
    SPDLOG_INFO(
        "[Windrose][R5UI][D3D12] copy frame={} modal={} blackout={} same_frame={} afr={} right={} ui_target=0x{:x} native=0x{:x} both_eyes={} reason={}",
        frame_count,
        modal_active,
        blackout_active,
        is_same_frame,
...
void VR::update_windrose_r5_ui_state(sdk::UGameEngine* engine) {
    const auto clear_state = [this](const char* reason) {
        const auto was_modal = m_windrose_r5_modal_ui_active.exchange(false, std::memory_order_relaxed) || m_windrose_r5_ui_scan.last_active;
        const auto was_blackout = m_windrose_r5_modal_scene_blackout_active.exchange(false, std::memory_order_relaxed) ||
            m_windrose_r5_ui_scan.last_scene_blackout_active;

        if (was_modal || was_blackout) {
            SPDLOG_INFO(
                "[Windrose][R5UI] modal transition active=false blackout=false reason={}",
                reason != nullptr ? reason : "cleared");
        }

        m_windrose_r5_ui_scan = {};
        std::scoped_lock _{m_windrose_r5_ui_copy_mutex};
        m_windrose_r5_has_ui_copy_sample = false;
    };
    ...
    if (m_rendering_method->value() != RenderingMethod::NATIVE_STEREO || is_using_afr()) {
        clear_state("not_native_stereo");
        return;
    }
    ...
    const auto now = std::chrono::steady_clock::now();
    const auto recently_seen_fullscreen_meta =
        m_windrose_r5_ui_scan.last_seen_fullscreen_meta.time_since_epoch().count() != 0 &&
        now - m_windrose_r5_ui_scan.last_seen_fullscreen_meta < std::chrono::milliseconds(1200);
    const auto recently_seen_negative_space =
        m_windrose_r5_ui_scan.last_seen_negative_space.time_since_epoch().count() != 0 &&
        now - m_windrose_r5_ui_scan.last_seen_negative_space < std::chrono::seconds(6);
    const auto recently_signaled_ui =
        ...
        recently_seen_fullscreen_meta ||
        recently_seen_negative_space ||
    ...
    m_windrose_r5_ui_scan.last_scan = now;

    const auto object_array = sdk::FUObjectArray::get();
    if (object_array == nullptr || IsBadReadPtr(object_array, sizeof(void*))) {
        clear_state("no_uobject_array");
        return;
    }

    const auto object_count = std::clamp(object_array->get_object_count(), 0, 2'000'000);
    if (object_count <= 0) {
        clear_state("empty_uobject_array");
        return;
    }

    if (m_windrose_r5_ui_scan.cursor < 0 ||
        m_windrose_r5_ui_scan.cursor >= object_count ||
        (m_windrose_r5_ui_scan.last_object_count != 0 && object_count < m_windrose_r5_ui_scan.last_object_count)) {
        m_windrose_r5_ui_scan.cursor = 0;
        m_windrose_r5_ui_scan.sweep_saw_mounter = false;
    ...
    while (scanned < scan_budget && m_windrose_r5_ui_scan.cursor < object_count) {
        auto* item = object_array->get_object(m_windrose_r5_ui_scan.cursor);
        ++m_windrose_r5_ui_scan.cursor;
        ++scanned;
        ...
        auto* object = reinterpret_cast<sdk::UObject*>(item->get_object());
        if (!is_valid_uobject_basic(object)) {
            continue;
        }
        ...
        uint32_t flags = 0;
        try {
            flags = object->get_object_flags();
        } catch (...) {
            continue;
        }

        if ((flags & 0x10) != 0) { // RF_ClassDefaultObject
            continue;
        }
        ...
        try {
            class_name = klass->get_name_safe();
        } catch (...) {
            class_name.clear();
        }

        if (!windrose_r5_ui_class_candidate(class_name)) {
            continue;
        }

        const auto full_name = get_object_full_name_safe_wide(object);
        if (!is_runtime_object_full_name(full_name)) {
            continue;
        }

        const auto combined_name = class_name + L" " + full_name;
        if (contains_case_insensitive(combined_name, L"BP_SimpleUIMounter_SC")) {
            m_windrose_r5_ui_scan.sweep_saw_mounter = true;
        }
        ...
        if (contains_case_insensitive(combined_name, L"BP_NegativeSpaceController_SC")) {
            ...
            m_windrose_r5_ui_scan.last_seen_negative_space = now;
            ...
            m_windrose_r5_ui_scan.last_seen_negative_space_sample = windrose_r5_ui_log_sample(full_name);
        }
        ...
        if (contains_case_insensitive(combined_name, L"R5Inventory") ||
            ...
            contains_case_insensitive(combined_name, L"InventoryItemsInfo") ||
            contains_case_insensitive(combined_name, L"MetaCharacter") ||
            contains_case_insensitive(combined_name, L"FullscreenMap") ||
            contains_case_insensitive(combined_name, L"Adventure") ||
            contains_case_insensitive(combined_name, L"Discovery") ||
            contains_case_insensitive(combined_name, L"Progression") ||
            contains_case_insensitive(combined_name, L"Talents") ||
            contains_case_insensitive(combined_name, L"PlayerFlagShip") ||
            contains_case_insensitive(combined_name, L"ShipManage") ||
            contains_case_insensitive(combined_name, L"Craft_") ||
            contains_case_insensitive(combined_name, L"Trade_") ||
            contains_case_insensitive(combined_name, L"ScenarioDialogue")) {
            m_windrose_r5_ui_scan.sweep_saw_inventory = true;
        }

        if (windrose_r5_ui_name_is_meta_menu_controller(combined_name)) {
            m_windrose_r5_ui_scan.sweep_saw_meta_menu_controller = true;
        ...
        if (is_active_fullscreen_meta) {
            ...
            m_windrose_r5_ui_scan.last_seen_fullscreen_meta = now;
            ...
            m_windrose_r5_ui_scan.last_seen_fullscreen_meta_sample = windrose_r5_ui_log_sample(full_name);
        }

        const auto is_modal_name = windrose_r5_modal_ui_name(combined_name);
        const auto is_active_modal = is_modal_name && windrose_r5_ui_instance_is_active(object);

        if (is_active_modal) {
            m_windrose_r5_ui_scan.sweep_saw_modal = true;
            ++m_windrose_r5_ui_scan.sweep_modal_hits;

            if (m_windrose_r5_ui_scan.sweep_sample.empty()) {
                m_windrose_r5_ui_scan.sweep_sample = windrose_r5_ui_log_sample(full_name);
            }
        }

        if (is_active_modal && windrose_r5_scene_blackout_ui_name(combined_name)) {
            m_windrose_r5_ui_scan.sweep_saw_scene_blackout = true;
            ++m_windrose_r5_ui_scan.sweep_scene_blackout_hits;

            if (m_windrose_r5_ui_scan.sweep_scene_blackout_sample.empty()) {
                m_windrose_r5_ui_scan.sweep_scene_blackout_sample = windrose_r5_ui_log_sample(full_name);
            }
        }
    }

    const auto completed_sweep = m_windrose_r5_ui_scan.cursor >= object_count;
    const auto recent_fullscreen_meta =
        m_windrose_r5_ui_scan.last_seen_fullscreen_meta.time_since_epoch().count() != 0 &&
        now - m_windrose_r5_ui_scan.last_seen_fullscreen_meta < std::chrono::milliseconds(1200);
    const auto recent_negative_space =
        m_windrose_r5_ui_scan.last_seen_negative_space.time_since_epoch().count() != 0 &&
        now - m_windrose_r5_ui_scan.last_seen_negative_space < std::chrono::seconds(6);

    const auto active_meta_menu_controller =
    ...
    const auto active = m_windrose_r5_ui_scan.sweep_saw_modal || active_meta_menu_controller || recent_fullscreen_meta;
    ...
    const auto force_negative_space =
        ...
        recent_negative_space &&
        (m_windrose_r5_ui_scan.forced_negative_space_controller != 0 ||
         m_windrose_r5_ui_scan.forced_negative_space_widget != 0);
    ...
    if (force_negative_space) {
        ...
        if (force_transition || should_keepalive) {
            ...
            if (controller != nullptr) {
                applied |= write_object_bool_property(controller, L"bEnableCapturing", true);
                applied |= write_object_bool_property(controller, L"bTickable", true);
                applied |= call_object_bool_param_function(controller, L"SetComponentTickEnabled", true);
                applied |= call_object_bool_param_function(controller, L"SetForceDisableCapture", false);
                if (force_transition) {
                    if (widget == nullptr) {
                        applied |= call_object_void_function(controller, L"Init");
                    }
                    applied |= call_object_void_function(controller, L"OnEnter");
                }

                if (widget == nullptr) {
                    widget = windrose_r5_negative_space_widget_from_controller(controller);
                    if (is_valid_uobject_basic(widget)) {
                        m_windrose_r5_ui_scan.forced_negative_space_widget = reinterpret_cast<uintptr_t>(widget);
                    }
                }
            }
            ...
            if (force_transition ||
                m_windrose_r5_ui_scan.last_seen_fullscreen_meta_sample != m_windrose_r5_ui_scan.last_forced_menu_sample) {
                m_windrose_r5_ui_scan.last_forced_menu_sample = m_windrose_r5_ui_scan.last_seen_fullscreen_meta_sample;
                SPDLOG_INFO(
                    ...
                    m_windrose_r5_ui_scan.last_seen_fullscreen_meta_sample.empty() ? "none" : m_windrose_r5_ui_scan.last_seen_fullscreen_meta_sample,
                    m_windrose_r5_ui_scan.last_seen_negative_space_sample.empty() ? "none" : m_windrose_r5_ui_scan.last_seen_negative_space_sample);
            }
    ...
    const auto changed =
        ...
        m_windrose_r5_ui_scan.sweep_saw_mounter != m_windrose_r5_ui_scan.last_saw_mounter ||
    ...
    const auto partial_signal_log_due =
        recent_fullscreen_meta &&
        !completed_sweep &&
        (m_windrose_r5_ui_scan.last_partial_signal_log.time_since_epoch().count() == 0 ||
         now - m_windrose_r5_ui_scan.last_partial_signal_log > std::chrono::seconds(2));

    if (changed || partial_signal_log_due) {
        if (partial_signal_log_due) {
            m_windrose_r5_ui_scan.last_partial_signal_log = now;
        }

        SPDLOG_INFO(
            "[Windrose][R5UI] modal transition active={} blackout={} partial={} mounter={} pool={} inventory={} meta_ctrl={} ns_ctrl={} ns_widget={} fullscreen_meta={} recent_fullscreen={} recent_ns={} hits={} blackout_hits={} fullscreen_hits={} sample=\"{}\" meta_sample=\"{}\" ns_sample=\"{}\" fullscreen_sample=\"{}\" blackout_sample=\"{}\"",
            active,
    ...
    m_windrose_r5_ui_scan.last_active = active;
    m_windrose_r5_ui_scan.last_scene_blackout_active = scene_blackout_active;
    m_windrose_r5_ui_scan.last_saw_mounter = m_windrose_r5_ui_scan.sweep_saw_mounter;
...
void VR::draw_hitch_diagnostics_ui() {
    if (!ImGui::TreeNode("Hitch Diagnostics")) {
        return;
    }

    if (!m_last_hitch_snapshot_summary.valid) {
        ImGui::TextWrapped("No hitch snapshot has been written this session.");
        ImGui::Text("Ring: sequence=%llu cursor=%zu wrapped=%s",
            (unsigned long long)m_hitch_snapshot_sequence,
            m_hitch_snapshot_cursor,
            m_hitch_snapshot_wrapped ? "yes" : "no");
        ImGui::TreePop();
        return;
    }

    const auto& summary = m_last_hitch_snapshot_summary;
    const auto& sample = summary.last_sample;
    const auto& d3d12 = sample.d3d12;

    ImGui::Text("Last snapshot: %s", summary.path.filename().string().c_str());
    ImGui::Text("Tick gap: %lld ms | suspected: %s | samples: %zu",
        (long long)summary.tick_gap_ms,
        summary.suspected_stall.c_str(),
        summary.sample_count);
    ImGui::Text("OpenXR ages: wait=%lldms begin=%lldms end=%lldms pose=%lldms",
        (long long)sample.xr_wait_age_ms,
        (long long)sample.xr_begin_age_ms,
        (long long)sample.xr_end_age_ms,
        (long long)sample.pose_update_age_ms);
    ImGui::Text("Frame state: synced=%s began=%s ready=%s submitted=%s controllers=%s",
        sample.frame_synced ? "yes" : "no",
        sample.frame_began ? "yes" : "no",
        sample.runtime_ready ? "yes" : "no",
        sample.submitted ? "yes" : "no",
        sample.using_controllers ? "yes" : "no");
    ImGui::Text("Stale pose: skips=%llu suppressed=%llu refresh=%llu/%llu/%llu last_pose_frame=%u",
        (unsigned long long)sample.stale_pose_older_frame_skip_count,
        (unsigned long long)sample.stale_pose_older_frame_skip_suppressed_count,
        (unsigned long long)sample.stale_pose_refresh_attempt_count,
        (unsigned long long)sample.stale_pose_refresh_success_count,
        (unsigned long long)sample.stale_pose_refresh_failed_count,
        sample.last_pose_update_frame_count);
    ImGui::Text("Recoveries: focused=%u forced=%u scene_capture_destroyed=%llu",
        sample.focused_frame_loop_recovery_count,
        sample.forced_frame_recovery_count,
        (unsigned long long)sample.scene_capture_destroyed_count);
    ImGui::Text("D3D12: game=%s ui=%s scene_capture=%s recreates=%llu reason=0x%x",
        d3d12.has_game_tex ? "yes" : "no",
        d3d12.has_ui_tex ? "yes" : "no",
        d3d12.has_scene_capture_tex ? "yes" : "no",
        (unsigned long long)d3d12.swapchain_recreate_count,
        d3d12.last_swapchain_recreate_reasons);
    ImGui::Text("D3D12 perf: on_frame avg=%.2f max=%.2f | ui_copy avg=%.2f max=%.2f | submit avg=%.2f max=%.2f",
        d3d12.perf_on_frame_avg_ms,
        d3d12.perf_on_frame_max_ms,
        d3d12.perf_ui_copy_avg_ms,
        d3d12.perf_ui_copy_max_ms,
        d3d12.perf_openxr_submit_avg_ms,
        d3d12.perf_openxr_submit_max_ms);
    ImGui::TextWrapped("Summary file: hitch_snapshots\\hitch_snapshot_latest_summary.json");
    ImGui::TreePop();
}
