...
constexpr std::string_view UI_INVERT_ALPHA_KEY{"UI_InvertAlpha"};
...
bool migrate_ui_invert_alpha(utility::Config& cfg) {
    auto apply_value = [&](float new_value) {
        ...
        cfg.set<float>(std::string{UI_INVERT_ALPHA_KEY}, clamped);
        return true;
    };

    auto safe_get_bool = [&]() -> std::optional<bool> {
        try {
            if (auto value = cfg.get<bool>(UI_INVERT_ALPHA_KEY)) {
                return *value;
            }
        } catch (...) {
        }
    ...
    auto safe_get_float = [&]() -> std::optional<float> {
        try {
            if (auto value = cfg.get<float>(UI_INVERT_ALPHA_KEY)) {
                return *value;
            }
        } catch (...) {
        }
    ...
    auto safe_get_string = [&]() -> std::optional<std::string> {
        try {
            return cfg.get(UI_INVERT_ALPHA_KEY);
        } catch (...) {
            return std::nullopt;
        }
    };
